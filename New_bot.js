...
player.on(AudioPlayerStatus.Idle, (oldState, newState) => {
  ...
  if (retryCount < MAX_RETRIES) {
    console.log(`Attempting to restart the stream. Retry ${retryCount + 1} of ${MAX_RETRIES}.`);
    retryCount++;
    ...
    console.log('Max retries reached. Setting idle timeout.');
...
client.on('messageCreate', async message => {
  ...
  if (message.content === '!startmusic' || message.content === '!start') {
    ...
    console.log(`Attempting to play stream: ${currentStation}`);
